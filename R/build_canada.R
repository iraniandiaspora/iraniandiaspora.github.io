...
PUMF_SRC_IMMIG <- paste0("Source: ", PUMF_LINK, " \u2014 2021 Canadian Census<br>",
  "Iran-born respondents only.<br>Pre-1995 immigration periods are grouped into multi-year bands.")
PUMF_SRC_EDUC_1G <- paste0("Source: ", PUMF_LINK, " \u2014 2021 Canadian Census<br>",
  "First generation ages 30+.")
PUMF_SRC_FOS_1G <- paste0("Source: ", PUMF_LINK, " \u2014 2021 Canadian Census<br>",
  "First generation ages 25+.")
...
PUMF_SRC_WORK_1G <- paste0("Source: ", PUMF_LINK, " \u2014 2021 Canadian Census<br>",
  "First generation ages 25+, by age group and gender.")
PUMF_SRC_WORK_2G <- paste0("Source: ", PUMF_LINK, " \u2014 2021 Canadian Census<br>",
  "Second generation ages 25+, all ages combined due to small sample size.")
PUMF_SRC_INCOME <- paste0("Source: ", PUMF_LINK, " \u2014 2021 Canadian Census<br>",
  "Ages 25\u201354 (prime working years).<br>Each decile holds 10% of all Canadian households, ranked by pre-tax household income.")
PUMF_SRC_INCOME_AGE <- paste0("Source: ", PUMF_LINK, " \u2014 2021 Canadian Census<br>",
  "First generation, ages 25\u201374.<br>Pre-tax personal income.")
...
function switchTab(tabId, btn, groupId) {
  ...
  // Resize any plotly charts in newly-visible tab
...
@media (max-width:900px) {
  ...
  .text-row, .chart-row { grid-template-columns:1fr; }
...
pop <- read.csv(file.path(CA_DIR, "population/iranian_population_final_breakdown_complete.csv"))
...
prov <- read.csv(file.path(CA_DIR, "immigration/settlement_by_province.csv"),
  check.names = FALSE)
...
prov_data <- read.csv(file.path(CA_DIR, "population/iranians_by_province.csv"))
...
library(sf)
sf_use_s2(FALSE)
ont_csds <- readRDS(file.path(CA_DIR, "population/ontario_csds_iranian.rds"))
...
ont_geojson_path <- file.path(CA_DIR, "population/ontario_csds.geojson")
...
writeLines(page_template("Canada: Defining the Population", paste0(
  ...
  '</ul>',
  '<p style="margin-top:8px; font-size:11px; color:#999;">Many people meet more than one criterion. The total count depends on whether Persian speakers who do not separately report Iranian birthplace or ethnicity are included&mdash;most are Canadian-born children of immigrants. The grid breaks down how the three criteria overlap.</p>',
  '</div>',
  '</div>',
...
lang <- read.csv(file.path(CA_DIR, "language/persian_language_patterns_FINAL.csv"))
relig <- read.csv(file.path(CA_DIR, "religion/religion_dashboard_ready.csv"))
...
lang <- lang %>% mutate(short_cat = case_when(
  grepl("^Persian \\(maintained\\)", persian_status) ~ "Persian mother tongue, Persian home",
  grepl("^Persian mother tongue", persian_status) ~ "Persian mother tongue, Eng/Fr home",
  grepl("^Iranian minority language", persian_status) ~ "Minority mother tongue, same at home",
  grepl("^Iranian minority mother tongue", persian_status) ~ "Minority mother tongue, Eng/Fr home",
  grepl("^Iranian minority moth", persian_status) ~ "Minority mother tongue, Eng/Fr home",
  grepl("^English \\(maintained\\)", persian_status) ~ "English mother tongue, English home",
  grepl("^Other", persian_status) ~ "Other languages",
  TRUE ~ persian_status
))
...
immig_annual <- read.csv(file.path(CA_DIR, "immigration/immigration_annual.csv"))
cit <- read.csv(file.path(CA_DIR, "citizenship/citizenship_status.csv"))
immig_cat <- read.csv(file.path(CA_DIR, "immigration/iranian_immigration_category_trends.csv"),
  check.names = FALSE)
...
immig_annual$hover <- ifelse(immig_annual$is_period,
  sprintf("<b>%s</b><br>%s total (%s/year avg)<br>Cumulative: %.1f%%",
...
ed1 <- read.csv(file.path(CA_DIR, "education/education_first_gen_30plus_FINAL.csv"))
fos <- read.csv(file.path(CA_DIR, "education/field_of_study_DASHBOARD_FINAL.csv"))
...
make_ca_educ_butterfly <- function(df, gen_label, id_prefix, height = "450px", source = PUMF_SRC_EDUC_1G) {
  educ_levels <- c("Graduate degree", "BA degree", "Less than BA degree")
  colors <- c("Graduate degree" = "#8bbdde", "BA degree" = "#2774AE", "Less than BA degree" = "#1a4e72")
...
# Field of study butterfly: single row "All adults 25+" per gender, 5 field categories
...
writeLines(page_template("Canada: Education", paste0(
  ...
  '<div class="chart-row">',
  '<div class="chart-card">',
  '<div class="section-title">First Generation: Educational Attainment by Gender</div>',
  make_ca_educ_butterfly(ed1, "1st Gen", "ca-ed1"),
  '</div>',
  '<div class="chart-card">',
  '<div class="section-title">First Generation: Fields of Study by Gender</div>',
...
work <- read.csv(file.path(CA_DIR, "work/industry_sectors_dashboard.csv"))
emp <- read.csv(file.path(CA_DIR, "work/employment_categories_dashboard.csv"))
...
writeLines(page_template("Canada: Work", paste0(
  ...
  '<div class="chart-card">',
  '<div class="section-title">First Generation: Work by Age and Gender</div>',
  '<div class="tab-bar">',
  ...
  '<div class="chart-card">',
  '<div class="section-title">Second Generation: Work by Gender</div>',
  '<div class="tab-bar">',
...
inc <- read.csv(file.path(CA_DIR, "work/income_distribution_FINAL.csv"))
inc_age <- read.csv(file.path(CA_DIR, "work/income_by_age_first_gen.csv"))
...
p_inc_decile <- plot_ly(data = inc1, x = ~short_label, y = ~percentage, type = "scatter",
  ...
  add_trace(x = inc1$short_label, y = rep(10, nrow(inc1)), type = "scatter",
    ...
    hoverinfo = "skip", showlegend = FALSE) %>%
  layout(
    title = list(text = "<b>Position in Canadian Income Distribution:<br>First Generation (Ages 25-54)</b>",
      font = list(size = 16, family = "Montserrat")),
...
p_inc_age <- p_inc_age %>% layout(
  barmode = "stack",
  title = list(text = "<b>Personal Income by Age:<br>First-Generation Iranian-Canadians</b><br><span style='font-size:11px;font-weight:normal;color:#666;'>Individual pre-tax income (not household)</span>",
    font = list(size = 16, family = "Montserrat")),
  xaxis = list(title = "", ticksuffix = "%", range = c(0, 105)),
  yaxis = list(title = "", categoryorder = "array", categoryarray = rev(age_levels_inc)),
  margin = list(t = 75, b = 60, l = 60), showlegend = FALSE,
  plot_bgcolor = "white", paper_bgcolor = "white",
  annotations = list(
    list(text = "Note: Personal income counts only the individual\u2019s earnings,<br>not the combined household total shown in the left chart.",
      x = 0.5, y = -0.15, xref = "paper", yref = "paper", showarrow = FALSE,
      font = list(size = 9, color = "#666"), xanchor = "center"))
) %>% config(displayModeBar = FALSE)
