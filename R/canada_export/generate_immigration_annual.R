...
period_counts <- period_data %>%
  group_by(yrim) %>%
  summarize(period_total = sum(weight), .groups = "drop") %>%
  left_join(period_map, by = "yrim") %>%
  mutate(
    count = period_total / period_years,  # average per year
    is_period = TRUE
  ) %>%
  select(year = mid_year, count, period_label, is_period, period_years, period_total)
...
annual_counts <- annual_data %>%
  ...
  select(year, count, period_label, is_period, period_years, period_total)
